...
class routing_rip:  
    ...
    def config_dv1(self,wan_net1,lan_net1):
        with self.ssh_manager.ssh_device1.invoke_shell() as channel:
            try:
                channel.send("enable network\n")
                channel.send("config terminal\n")
                channel.send("router rip\n")
                channel.send(f"network {wan_net1}")
                channel.send(f"network {lan_net1}")
                print("config1 done!!!")
            except Exception as e:
                print(f"Lỗi trong bài kiểm thử 1: {str(e)}")

    def config_dv2(self,wan_net2,lan_net2):
        with self.ssh_manager.ssh_device2.invoke_shell() as channel:
            try:
                channel.send("enable network\n")
                channel.send("config terminal\n")
                channel.send("router rip\n")
                channel.send(f"network {wan_net2}")
                channel.send(f"network {lan_net2}")
                print("config2 done!!!")
            except Exception as e:
                print(f"Lỗi : {str(e)}")
...
def main():
    ...
    static_router = routing_rip(ssh_manager)
